...
<div class="flex flex-col gap-2 p-2 w-full max-w-sm mx-auto">
	<Badge
		class="cursor-pointer bg-blue-500 text-lg w-full justify-center text-white dark:bg-blue-600 hover:bg-blue-600 transition-colors"
		variant="outline"
		onclick={() => {
	...
	<DropdownMenu.Root>
		<DropdownMenu.Trigger class="outline-none w-full">
			<Badge class="cursor-pointer w-full justify-center bg-blue-500 text-lg text-white dark:bg-blue-600 hover:bg-blue-600 transition-colors" variant="outline"
				><span>Account Level:</span>
				{data.accountData.account_level}</Badge
			>
		</DropdownMenu.Trigger>
		<DropdownMenu.Content class="popup-animate z-50 min-w-[12rem] rounded-md border bg-popover p-1 text-popover-foreground shadow-md outline-none data-[side=bottom]:slide-in-from-top-2">
			{#if data.matchesData}
				<DropdownMenu.Group>
					...
					{#each Object.entries(getClusterCount(data.matchesData)).sort((a, b) => Number(b[1]) - Number(a[1])) as [cluster, count]}
						<DropdownMenu.Item class="relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none">{cluster}: {count}</DropdownMenu.Item>
					{/each}
				</DropdownMenu.Group>
				<DropdownMenu.Separator class="-mx-1 my-1 h-px bg-muted" />
				<DropdownMenu.Item class="relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none">Total Matches: {getTotalMatches(data.matchesData)}</DropdownMenu.Item>
				<DropdownMenu.Item class="relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none">Headshot: {getHsPercentage(data.matchesData)}%</DropdownMenu.Item>
			{/if}
	...
	>
		<Badge
			...
			class="block w-full text-left cursor-pointer truncate overflow-hidden bg-blue-500 text-lg text-white dark:bg-blue-600 hover:bg-blue-600 transition-colors"
			variant="outline"
			onclick={() => {
	...
	<div class="flex gap-2 justify-between w-full">
		{#if data.mmrData?.current}
			<Badge 
				class="flex-1 justify-center cursor-pointer bg-blue-500 text-lg text-white dark:bg-blue-600 hover:bg-blue-600 transition-colors flex items-center gap-1" 
				variant="outline"
				onclick={() => {
					initialSeasonShort = '';
					isMmrModalOpen = true;
		...
		{#if data.mmrData?.peak}
			<Badge 
				...
				title="Click to view full Competitive History"
			>
				<span>Pk[{data.mmrData.peak.season.short}]:</span>
				<img
